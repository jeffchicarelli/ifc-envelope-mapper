...
{
    private const double PARALLEL_THRESHOLD = 0.99;

    /// <summary>
    /// Finite square quad sampled from this (infinite) plane, useful for visualising fitted planes. The quad has edge length
    /// <paramref name="displaySize"/> and is centred at the plane's foot of perpendicular from the world origin.
    /// </summary>
    public static DMesh3 ToQuadMesh(this Plane3d plane, double displaySize)
    {
        ...
        var up = Math.Abs(normal.Dot(Vector3d.AxisY)) < PARALLEL_THRESHOLD ? Vector3d.AxisY : Vector3d.AxisX;
