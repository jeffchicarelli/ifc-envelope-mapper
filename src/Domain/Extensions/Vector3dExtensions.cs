...
{
    /// <summary>Best-fit plane through the given points via PCA. Minimises the sum of squared perpendicular distances from every point.</summary>
    ...
    /// <summary>UV-sphere mesh centred at this point.</summary>
    ...
    public static DMesh3 ToSphere(this Vector3d center, double radius, int rings = 8, int sectors = 12)
    {
        ...
        for (var r = 0; r <= rings; r++)
        {
            for (var s = 0; s < sectors; s++)
            {
                ...
                indices[(r * sectors) + s] = mesh.AppendVertex(new Vector3d(center.x + (radius * Math.Sin(phi) * Math.Cos(theta)),
