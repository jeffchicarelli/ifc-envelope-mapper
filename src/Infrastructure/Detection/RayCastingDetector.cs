...
/// <summary>Detects exterior elements by per-triangle ray casting (Ying et al. 2022).</summary>
/// <remarks>
/// Algorithm:
/// <list type="number">
...
/// <item>
/// For every triangle of every element, cast <c>N</c> rays from <c>centroid + ε·normal</c> along the outward normal, jittered by ±
/// <c>jitterDeg</c>. A ray "escapes" if it hits nothing or hits a triangle owned by the same element.
/// </item>
...
public sealed class RayCastingDetector : IEnvelopeDetector
{
    ...
    public RayCastingDetector(int numRays = 8, double jitterDeg = 5.0, double hitRatio = 0.5, IFaceExtractor? faceExtractor = null)
    {
        _numRays = numRays;
    ...
    public StrategyConfig Config => new(null, _numRays, (_jitterRad * 180.0) / Math.PI, _hitRatio);
    ...
    public DetectionResult Detect(IEnumerable<IElement> elements)
    {
        ...
        var bvh = new DMeshAABBTree3(globalMesh, true);

        Log.LogInformation("Built BVH on {Triangles} triangles ({Elements} elements)", globalMesh.TriangleCount, elementsList.Count);
        ...
        Log.LogInformation("Ray casting done: {Ext} exterior / {Int} interior ({Rays} rays cast)", exteriorIds.Count,
                           elementsList.Count - exteriorIds.Count, totalRays);
