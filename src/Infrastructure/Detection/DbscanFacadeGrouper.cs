...
/// <summary>
/// Groups exterior <see cref="Face"/>s from an <see cref="Envelope"/> into <see cref="Facade"/>s using DBSCAN over the Gauss sphere followed by a
/// QuikGraph connected-components split inside each orientation cluster.
/// <list type="bullet">
/// <item>
/// <description>
/// DBSCAN clusters faces by angular distance between normals (ε = <see cref="EpsilonDeg"/>°, metric = arccos(n₁·n₂)). Noise
/// faces — those that do not belong to any dense cluster — are silently discarded; they represent ambiguously-oriented geometry.
/// </description>
/// </item>
/// <item>
/// <description>
/// Within each orientation cluster, QuikGraph separates physically disconnected groups (e.g. a north-facing wall on opposite
/// sides of a courtyard) into independent <see cref="Facade"/>s via undirected connected-components over a centroid-distance adjacency
/// graph.
/// </description>
/// </item>
/// <item>
/// <description>
/// Facades are returned ordered by <see cref="Facade.AzimuthDegrees"/> ascending — facade-00 is always the most-northward
/// orientation.
/// </description>
/// </item>
/// </list>
...
{
    private const double MIN_FACADE_AREA = 1e-10;

    // ── Debug ───────────────────────────────────────────────────────────────

    // 8-colour palette cycling over facade index — wraps for models with >8 facades.
    private static readonly string[] _debugPalette = ["#00cc44", "#cc0000", "#0088ff", "#ff8800", "#8800cc", "#00cccc", "#cccc00", "#888888"];

    /// <summary>Creates a grouper with the given DBSCAN and adjacency parameters.</summary>
    public DbscanFacadeGrouper(double epsilonDeg = 15.0, int minFaces = 3, double adjacencyM = 3.0)
    {
    ...
    /// <summary>
    /// Maximum centroid distance (metres) for two faces to be considered adjacent in the spatial connectivity graph. Default 3.0 m — larger than
    /// typical wall thickness (~0.3 m) but smaller than typical facade-to-facade gap.
    /// </summary>
    ...
    public IReadOnlyList<Facade> Group(Envelope envelope)
    {
        ...
        Log.LogInformation("DBSCAN: {ClusterCount} orientation clusters, {NoiseCount} noise faces discarded", clusters.Count, noiseCount);
        ...
        foreach (var clusterFaces in clusters)
        {
            ...
            foreach (var component in labels.GroupBy(kv => kv.Value).Select(g => (IReadOnlyList<Face>)g.Select(kv => kv.Key).ToList()))
            {
                var dominant = DominantNormal(component);
                var azimuth = Azimuth(dominant);

                facades.Add(new Facade($"facade-{facadeIdx++:D2}", envelope, component, new DMesh3(), dominant, azimuth));
            }
        }
    ...
    private (List<List<Face>> Clusters, int NoiseCount) GaussSphereDbscan(IReadOnlyList<Face> faces)
    {
    ...
    private void ExpandCluster(
        IReadOnlyList<Face> faces, int seedIdx, List<int> seeds, List<Face> cluster, bool[] inCluster, double epsilonRad, bool[] visited)
    {
    ...
    private static void AddToCluster(IReadOnlyList<Face> faces, int idx, List<Face> cluster, bool[] inCluster)
    {
        inCluster[idx] = true;
