...
/// <summary>
/// RGBA colour for debug emissions. Stored as four bytes; equality is by value. Failures (bad hex input) surface at <see cref="FromHex"/>
/// rather than deep inside the GLB encoder, so the offending call site is the one that throws.
/// </summary>
public readonly record struct Color(byte R, byte G, byte B, byte A = 255)
{
    ...
    /// <summary>Parses <c>#RRGGBB</c> or <c>#RRGGBBAA</c>. Leading <c>#</c> is optional. Throws <see cref="FormatException"/> on any other shape.</summary>
    public static Color FromHex(string hex)
    {
        ...
        var a = span.Length == 8 ? byte.Parse(span[6..8], NumberStyles.HexNumber, CultureInfo.InvariantCulture) : (byte)255;
