...
/// <summary>
/// Domain entity wrapping an <see cref="IIfcProduct"/>. Carries lazy mesh and bounding box, a spatial context bundle, and an optional list of
/// children for composite IFC products (e.g. IfcCurtainWall, IfcRoof). Equality is identity by <see cref="GlobalId"/>.
/// </summary>
public class Element : IElement, IProductEntity, IEquatable<Element>
{
    ...
    public Element(IfcProductContext ctx, Lazy<DMesh3> lazyMesh, Lazy<AxisAlignedBox3d> lazyBbox)
    {
        _ctx = ctx;
        _lazyMesh = lazyMesh;
        _lazyBbox = lazyBbox;
    ...
    /// <summary>Sharing constructor: a derived class reuses the lazy state of an existing instance, so the same cached mesh/bbox is shared.</summary>
    ...
    {
        return _lazyMesh.Value;
    }

    /// <inheritdoc/>
    public AxisAlignedBox3d GetBoundingBox()
    {
        return _lazyBbox.Value;
    }
    ...
    /// <inheritdoc/>
    public IIfcSite? GetIfcSite()
    {
        return _ctx.Site;
    }

    /// <inheritdoc/>
    public IIfcBuilding? GetIfcBuilding()
    {
        return _ctx.Building;
    }
