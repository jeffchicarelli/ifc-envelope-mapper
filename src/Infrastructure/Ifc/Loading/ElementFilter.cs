...
/// <summary>
/// IFC-type allow-list controlling which entities the loader emits. The default set covers the constructive element types relevant to envelope
/// detection; callers can pass a custom set (e.g., for specialized IFC profiles or reduced-scope runs).
/// </summary>
public sealed class ElementFilter
{
    private static readonly IReadOnlySet<string> _defaultTypes = new HashSet<string>(StringComparer.Ordinal)
    {
    ...
    {
        _types = types ?? _defaultTypes;
    }

    /// <summary><c>true</c> when <paramref name="ifcType"/> is in the allow-list.</summary>
    public bool Include(string ifcType)
    {
        return _types.Contains(ifcType);
    }
}
