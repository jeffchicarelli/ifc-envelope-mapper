...
using Element = IfcEnvelopeMapper.Ifc.Domain.Element;

/// <summary>
/// Unit tests for <see cref="MetricsCalculator.Compute"/>. Uses real
/// <c>Element</c>s from <c>duplex.ifc</c> to satisfy
/// <see cref="ElementClassification.Element"/> + <c>GlobalId</c> matching;
/// the actual classification verdicts and ground-truth labels are synthesised
/// in-test so each scenario is fully controlled.
/// </summary>
...
public sealed class MetricsCalculatorTests : IfcTestBase
{
    ...
    {
        // Pick four real elements and assign one to each cell of the confusion matrix.
        var (eTp, eFp, eFn, eTn) = (Model.Elements[0], Model.Elements[1], Model.Elements[2], Model.Elements[3]);
    ...
    {
        // IsExterior == null means "unknown" — these rows must not contribute
        // to any cell. The classifier was forced to predict; the GT abstained.
    ...
    {
        // GT is the source of truth for which elements are scored. If the
        // classifier predicts on an element absent from GT, we don't count it
        // (rather than guess).
        var element = Model.Elements[0];
    ...
    {
        var elements = Model.Elements.Take(5).ToList();
    ...
    {
        var elements = Model.Elements.Take(3).ToList();
        ...
        // No actual positives in GT → recall denominator is zero → NaN.
        double.IsNaN(counts.Recall).Should().BeTrue();
    }
