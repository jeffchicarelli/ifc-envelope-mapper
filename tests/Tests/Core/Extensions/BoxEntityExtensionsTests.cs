...
/// <summary>
/// Unit tests for <see cref="BoxEntityExtensions.BoundingBox"/>. Uses a tiny
/// in-test <c>IBoxEntity</c> stub so the test does not depend on
/// <c>Element</c> (which can only be constructed by the loader).
/// </summary>
public class BoxEntityExtensionsTests
{
    ...
    {
        // Regression guard: an early naive implementation that initialised
    ...
    {
        // Select() yields a non-IList sequence, forcing the `?? .ToList()`
        // fallback. The yields counter proves the source is enumerated only
        // once even though the implementation indexes by position.
        var yields = 0;
