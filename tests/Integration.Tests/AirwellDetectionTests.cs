...
///   An airwell (vertically open shaft) is exterior space.
...
///
/// Models used (all small, purpose-built, in data/models/airwell/):
/// <list type="bullet">
...
public sealed class AirwellDetectionTests : IfcTestBase
{
    ...
    {
        var result = new VoxelFloodFillDetector(voxelSize: 0.25).Detect(LoadModel(modelFile).Elements);
        FindClassification(result, globalId).IsExterior.Should().BeTrue();
    }

    /// <summary>
    /// The most important test in this class. Element 2yrIoyurn0lPXgxy2$CNaj in
    /// control.ifc bounds the airwell shaft on one face and the enclosed building
    /// interior on the other. Because the shaft is open to the sky, the exterior
    /// flood-fill reaches it from above — the element's voxels touch exterior voxels
    /// and MUST be classified exterior.
    /// </summary>
    [Fact]
    public void VoxelDetect_Control_AirwellAdjacentWall_IsExterior()
    {
        var result = new VoxelFloodFillDetector(voxelSize: 0.25).Detect(LoadModel("airwell/control.ifc").Elements);
    ...
    /// <list type="bullet">
    ///   <item>with one balcony.ifc — 1wPpdLZdX3efKBKkOInIeg, 1wPpdLZdX3efKBKkOInIef</item>
    ///   <item>small recess.ifc    — 0mEhezLTj6DBaCuBWyJoXC</item>
    /// </list>
    ...
    /// hit the opposite wall and are blocked. Contrast with
    /// VoxelDetect_Control_AirwellAdjacentWall_IsExterior: this divergence is a key
    ...
    /// in control.ifc. This disagreement is the central dissertation finding for this
    /// element class.
    /// </summary>
    [Fact]
